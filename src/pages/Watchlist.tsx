...
function Watchlist() {
  ...
  const handleRemove = async (id: number, e: React.MouseEvent) => {
    ...
    try {
      await api.post("/watchlist/remove", { movie_id: id });
  ...
  const handleClearAll = async () => {
    if (window.confirm("Are you sure you want to clear your entire watchlist?")) {
      try {
        await api.post("/watchlist/clear");
        setWatchlist([]);
      } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-[#0B0F19] text-white">
        ...
        <div className="flex flex-col items-center justify-center pt-52">
          ...
          <p className="text-slate-400 font-medium">Loading your watchlist...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-white">
      ...
      <main className="pt-32 pb-16 px-6 md:px-12 max-w-[1600px] mx-auto animate-in fade-in duration-500">
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 rounded-2xl px-6 py-4 text-sm font-semibold mb-8 text-center animate-in fade-in duration-300">
            {error}
          </div>
        )}
        {/* Header Section */}
