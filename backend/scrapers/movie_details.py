...
OUTPUT_DIR = os.path.join(_DATA_DIR, "movie_details")
...
def find_sql_files() -> List[str]:
    # Look in the backend/data directory for SQL dumps
    search_dir = _DATA_DIR
    return sorted([f for f in os.listdir(search_dir) if f.lower().endswith(".sql")])
...
def choose_csv_file() -> str:
    # Look in the backend/data directory for CSV files
    search_dir = _DATA_DIR
    files = sorted([f for f in os.listdir(search_dir) if f.lower().endswith(".csv")])
    ...
    return os.path.join(search_dir, files[index - 1])
...
async def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not sql_files:
                print(f"No .sql files found in {_DATA_DIR}.")
                continue
